...
language.Add( "Tool.daktanklinker.0", "Left click to select the motor, AL clip, or turret motor. Right click on the fuel, gun, or turret control. Also links crew members to things." )
end
...
function TOOL:LeftClick( trace )
	if CurTime()>self.LastLeftClick then
		...
		if(string.Explode("_",Target:GetClass(),false)[1] == "dak") then
			if Target:GetClass() == "dak_temotor" or Target:GetClass() == "dak_teautoloadingmodule" or Target:GetClass() == "dak_turretmotor" or Target:GetClass() == "dak_crew" then
				self.Ent1 = Target
	...
	if CurTime()>self.LastRightClick then
		...
		if not(self.Ent1==NULL) then
			if self.Ent1:GetClass() == "dak_crew" then
				if(Target:GetClass() == "dak_temotor" or Target:GetClass() == "dak_tegun" or Target:GetClass() == "dak_teautogun") then
					if(Target:GetClass() == "dak_temotor" or Target:GetClass() == "dak_tegun") then
						self.Ent2 = Target
...
function TOOL.BuildCPanel( panel )
	...
	DLabel:SetText( "This tool just links motors to fuel, clips to autoloaders, and turret motors to turret controls, also links crew to things. Ammo is automatically found on the contraption by the gun." )
