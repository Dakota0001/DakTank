...
local function NetworkSize(Entity, Player)
	...
	if timer.Exists("Network Sizes") then return end

	timer.Create("Network Sizes", 0, 1, SendQueued)
end
...
function ENT:GetOriginalSize()
	...
	if Changed or not self.OriginalSize then
		...
		self.OriginalSize = DTTE.Models.GetSize(Model)
	end
...
do -- AdvDupe2 duped parented ammo workaround
	...
	hook.Add("AdvDupe_FinishPasting", "Parented Scalable Ent Fix", function(DupeInfo)
