...
SWEP.Instructions = "totally not a famas, 5.56mm pen"
...
function SWEP:PrimaryAttack()
	if self.PrimaryLastFire+self.PrimaryCooldown<CurTime() then
		if self.Weapon:Clip1() > 0 then
			...
			local FinalRecoil = BaseRecoil * self.ShotCount * 100 * ShotForce * IsMoving * IsCrouch * (self.SpreadStacks/1) --have spread stacks so first few shots are sorta accurate
